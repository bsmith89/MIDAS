...
def bowtie2_align(args, bt2_db_dir, bt2_db_name, sort_aln=False, verbose=False):
    """
    ...
    try:
        bt2_command = f"bowtie2 --no-unal -x {bt2_db_dir}/{bt2_db_name} {max_reads} {maybe_verbose} --{aln_mode} --{aln_speed} --threads {num_physical_cores} -q {r1} {r2}"
        if sort_aln:
